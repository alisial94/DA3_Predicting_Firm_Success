...
dir <- "C:\\Users\\Rauhan Nazir\\Documents\\R_Codes\\DA3\\Assignment 3\\"
getwd()
...
# Load the data
data <- readRDS("Assignment 3/Data/Clean/bisnode_firms_clean.rds")
...
#saveRDS(data_train,"Data/Clean/data_train_f.RDS")
#saveRDS(data_holdout,"Data/Clean/data_holdout_f.RDS")

data_train <- readRDS("Assignment 3/Data/Clean/data_train_f.RDS")

data_holdout <-readRDS("Assignment 3/Data/Clean/data_holdout_f.RDS") 
...
write.csv(lasso_coeffs, paste0(output, "lasso_logit_coeffs_f.csv"))
...
logit_summary1_f <- logit_summary1
...
roc_2_f <- createRocPlot(roc_obj_holdout, "best_no_loss_roc_plot_holdout")
roc_2_f
...
logit_summary2_f <- logit_summary2
...
roc_3_f<- createLossPlot(roc_obj, best_treshold, "rf_p_loss_plot")
roc_4_f <- createRocPlotWithOptimal(roc_obj, best_treshold, "rf_p_roc_plot")
...
summary_results_f <- summary_results

model_names <- c("Logit X2", "Logit X3", "Logit X4",
                 "Logit LASSO","RF probability")
summary_results <- summary_results %>%
  filter(rownames(.) %in% c("X2", "X3","X4", "LASSO", "rf_p"))
...
cali_f<- create_calibration_plot(data_holdout, 
